<template>
	<div class="page">
	  	<div class="page-content" @click="this.isAreaPicker == false">
			  ...
			  <button class="button button-icon headWord" slot="right" @click="editThisAddress">保存</button>
			</von-header>
...
<script>
	...
	export default{
		...
		data(){
			return{
				id:this.$route.query.address.id,
				username:this.$route.query.address.name,
				phone:this.$route.query.address.mobile,
				district:this.$route.query.address.area,
				address:this.$route.query.address.address,
				isDefault:this.$route.query.address.is_default || 0,
				disable:true,
		  		pickData: {
			        data1: provs_data,
			        data2: citys_data,
		...
		methods:{
		    ...
		    editThisAddress(){ //编辑地址后保存
	    		...
	    		that.$axios({
	    			method:'post',
	    			url:'http://www.paipaiwang.com.cn/api/hompage/addressedit',
	    			data:{
	    				token:localStorage.getItem('token'),
	    				id:that.$route.query.address.id,
	    				name:that.username,
	    				mobile:that.phone,
	    				area:that.district,
