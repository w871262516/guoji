	  	...
	  	<div class="page-content">
		  	...
		  	<list class="list-ios item-icon-right" v-if="myAllAddressList.length>0">
				<item v-for="(data,index) in myAllAddressList" :key="index">
					<router-link :to="{path:'/mine/salerCenter/salerEditAddress',query:{address:data}}" class="section">
						<div>
							<span>{{data.name}}</span>
...
<script>
	export default{
		...
		methods:{
			...
			goAddAddress(){
				this.$router.push('/mine/salerCenter/salerAddAddress')
			},
			getMyAllAddress(){
				let that = this
	    		that.$axios({
